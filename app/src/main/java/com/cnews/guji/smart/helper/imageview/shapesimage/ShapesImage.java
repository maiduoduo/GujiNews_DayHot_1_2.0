package com.cnews.guji.smart.helper.imageview.shapesimage;
        ...
        switch (checkShape) {
            case CUSTOM:
                if (Build.VERSION.SDK_INT >= 21)
                    mMaskDrawable = getResources().getDrawable(R.drawable.shape_frontnews_image, null);
                else
                    mMaskDrawable = getResources().getDrawable(R.drawable.shape_frontnews_image);
                if (mMaskDrawable != null) {
                    mMaskDrawable.setCallback(this);
                }
                break;
...
//                cacheCanvas.saveLayer(mBoundsF, mMaskedPaint, Canvas.HAS_ALPHA_LAYER_SAVE_FLAG | Canvas.FULL_COLOR_LAYER_SAVE_FLAG);
                cacheCanvas.saveLayer(mBoundsF, mMaskedPaint, 0);
